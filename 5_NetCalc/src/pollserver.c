...
ssize_t send_all(int sockfd, const void *buf, size_t len)
{
    ...
    {
        ssize_t sent
            = send(sockfd, (const char *)buf + total_sent, len - total_sent, 0);
        if (sent <= 0)
            return -1;
        total_sent += sent;
    }
        ...
        for (int i = 0; i < fd_count; i++)
        {
            ...
            if (pfds[i].revents & (POLLIN | POLLHUP))
            {
                if (pfds[i].fd == listener)
                {
                    ...
                    newfd   = accept(
                        listener, (struct sockaddr *)&remoteaddr, &addrlen);
                ...
                else
                {
                    ...
                    if (process_buffer((uint8_t *)data_buf,
                                       payload_size,
                                       &response_buf,
                                       &response_size)
                        != 0)
