...
hash_table_t *hash_table_init(uint32_t size, FREE_F customfree)
{
    ...
    if (DEBUG)
    {
        ...
        {
            fprintf(stderr,
                    "[-] hash_table_init fail; customfree does not exist.\n");
            // return NULL; // causes errors when uncommented
        }
    }

    // Allocate the hash table via malloc
