...
int list_foreach_call(list_t *list, ACT_F action_function)
{
    ...
    if (!action_function)
    {
        fprintf(
            stderr,
            "[-] Linked list missing action_function in list_foreach_call.\n");
        return 1;
    }
...
list_node_t *list_find_first_occurrence(list_t *list, void **search_data)
{
    ...
    {
        fprintf(stderr,
                "[-] Linked list nonexistent in list_find_first_occurrence.\n");
        return NULL;
    }
...
list_t *list_find_all_occurrences(list_t *list, void **search_data)
{
    ...
    {
        fprintf(stderr,
                "[-] Linked list nonexistent in list_find_all_occurrences.\n");
        return NULL;
    }
    // Instnatiate list to hold list of matches
    list_t *receiving_list = malloc(sizeof(list_t));
    // Initialize list parameters for the list to return
    ...
    while (curr)
    {
        if (node_compare(*search_data, curr))
        {
            ...
            {
                // Assign head and tail when list starts empty
                receiving_list->head = new_node;
                receiving_list->tail = new_node;
            }
            else
            {
                // Shift tail when list size >= 1
